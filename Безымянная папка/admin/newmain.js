...
{
	var string="<li onClick='TableObject.CreateTypeObject()' class='LiMenuTwo' id='Id_NTO-new'>Новые класс</li> ";
	for(value in data)
	{
		string+="<li onClick='TableObject.ViewTableClassObjects("+data[value]['id']+")' class='LiMenuTwo'  id='Id_NTO-"+data[value]['id']+"' >"+data[value]['name']+"</li>";
...
var TableObject=(function(){
	var NumNewFullProfile=0;
	var NumNewSmallProfile=0;
	var NumFullProfile=0;
	var NumSmallProfile=0;
	var Id_NameTypeObject;
	var StatusSave=true;
	var NewTableClassObject=function(){
	var string='';
	...
	string+="<div align='center' class='TrSmall'><button onClick='TableObject.NewSmallProfile()' id='CreateSP'>Добавить</button></div>";

	string+="<div align='center' id='TrFullOne' class='TrFull'><span class='NameSpan' >Подробные парамметры</span></div>";
	string+="<div align='center' class='TrFull'><button class='ButtonClass' id='CreateFP' onClick='TableObject.NewFullProfile()'>Добавить</button></div>";
	string+="<div align='center' class='TrPanel'><table ><tr><td><button id='SeveAll' onClick='TableObject.SeveAllTypeObject()' >Сохранить</button></td>";
	string+="<td><button id='RemoveTypeObject' class='ButtonClass' onClick='massage.CreateWarningMassage(\"TableObject.RemoveAllTypeObject()";
...
SetId_NTO:function(id){
	...
	Id_NameTypeObject=parseInt(id);
},

ViewTableClassObjects:function(Id_NTO){
	NewTableClassObject();
	NumNewSmallProfile=0;
	NumNewFullProfile=0;
	NumFullProfile=0;
	NumSmallProfile=0;
	...
	var GetSmall=function(data)
	{
		...
		for(var value in data)
		{
			...
			string+=" onClick='massage.CreateWarningMassage(\"TableObject.RemoveSmallProfile("+data[value]['id']+")\",\"Удалить параметр\")'/></div>";
			NumSmallProfile++;
		}
	...
	var GetFull=function(data){
		...
		for(var value in data)
		{
			...
			string+=" onClick='massage.CreateWarningMassage(\"TableObject.RemoveFullProfile("+data[value]['id']+")\",\"Удалить параметр\")'/></div>";
			NumFullProfile++;
	...
	Color();
	var callback=function(data){
		TableObject.SetId_NTO(data);
		TableObject.ViewTableClassObjects(Id_NameTypeObject);
};
	...
	NewFullProfile:function(){
		var string='';
			string+="<div align='center' class='TrFull'><input type='text' class='NewFP' id='Text-FP-News-"+NumNewFullProfile;
			string+="' value='Ввидите имя пареметра "+NumNewFullProfile+"'/><img class='edite' aling='right' ";
			string+="src='img/edite_fon.png' id='RemoveNewFullProfile-"+NumNewFullProfile+"'";
			string+=" onClick='massage.CreateWarningMassage(\"TableObject.RemoveNewFullProfile("+NumNewFullProfile+")\",";
	...
	NewSmallProfile:function(){
		var string='';
		    string+="<div align='center' class='TrSmall'><input type='text' class='NewSP' id='Text-SP-News-"+NumNewSmallProfile;
			string+="' value='Ввидите имя пареметра "+NumNewSmallProfile+"'/><img class='edite' aling='right' ";
			string+="src='img/edite_fon.png' id='RemoveNewSmallProfile-"+NumNewSmallProfile+"'";
			string+=" onClick='massage.CreateWarningMassage(\"TableObject.RemoveNewSmallProfile("+NumNewSmallProfile+")\",";
			string+="\"Удалить параметр\")'/></div>";
			NumNewSmallProfile++;
	...
	SeveAllTypeObject:function(){
		...
		for(var i=0;i<NumNewFullProfile;i++)
		{
			TypeObject.SetFullProfile(TableObject.RepairStatus,arrayFP[i].value,Id_NameTypeObject,"local");
		}
		for(var i=0;i<NumNewSmallProfile;i++)
		{
			TypeObject.SetSmallProfile(TableObject.RepairStatus,arraySP[i].value,Id_NameTypeObject);
		}
			arrayFP=Jquery.SetArrayElement(null,"OldFull");
			arraySP=Jquery.SetArrayElement(null,"OldSmall");

			for(var i=0;i<NumFullProfile;i++)
		{
			TypeObject.ReloadeFullProfile(TableObject.RepairStatus,arrayFP[i].value,Id_NameTypeObject);
		}
		for(var i=0;i<NumSmallProfile;i++)
		{
			TypeObject.ReloadeSmallProfile(TableObject.RepairStatus,arraySP[i].value,this.ParseId(arraySP[i].id));
		}
			TypeObject.ReloadeTypeObject(callback,Jquery.GetVal("NameTypeObject",null),Id_NameTypeObject);
			this.ViewTableClassObjects(Id_NameTypeObject);
	},
	RemoveAllTypeObject:function(){
		var callback=function(data){
			...
			TypeObject.RemoveTypeObject(callback,Id_NameTypeObject);
			this.ViewTableClassObjects(Id_NameTypeObject);
